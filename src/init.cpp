...
void Shutdown()
{
    ...
    RenameThread("MedicCoin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: MedicCoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: MedicCoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeMedicCoinamount=<n> " + _("Keep N MedicCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MedicCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MedicCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("MedicCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "MedicCoin server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MedicCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart MedicCoin to complete") << "\n";
    ...
    nAnonymizeMedicCoinAmount = GetArg("-anonymizeMedicCoinamount", 0);
    if(nAnonymizeMedicCoinAmount > 999999) nAnonymizeMedicCoinAmount = 999999;
    if(nAnonymizeMedicCoinAmount < 2) nAnonymizeMedicCoinAmount = 2;
    ...
    LogPrintf("Anonymize MedicCoin Amount %d\n", nAnonymizeMedicCoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1MEDIC+1000 == (.1MEDIC+100)*10
       10MEDIC+10000 == (1MEDIC+1000)*10
    */
