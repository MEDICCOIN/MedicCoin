...
int nAnonymizeMedicCoinAmount = 1000;
...
#else
    const char* pszModule = "MedicCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MedicCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MedicCoin
    // Mac: ~/Library/Application Support/MedicCoin
    // Unix: ~/.MedicCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MedicCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "MedicCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "MedicCoind.pid"));
