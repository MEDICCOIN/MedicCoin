...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeMedicCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeMedicCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeMedicCoinAmount = strAnonymizeMedicCoinAmount.remove(strAnonymizeMedicCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMedicCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeMedicCoinAmount*COIN) nMaxToAnonymize = nAnonymizeMedicCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeMedicCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeMedicCoinAmount));
        strAnonymizeMedicCoinAmount = strAnonymizeMedicCoinAmount.remove(strAnonymizeMedicCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMedicCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeMedicCoinAmount == 0){
