...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeMedicCoinAmount"))
        settings.setValue("nAnonymizeMedicCoinAmount", 1000);
    nAnonymizeMedicCoinAmount = settings.value("nAnonymizeMedicCoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeMedicCoinAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeMedicCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeMedicCoinAmount:
            return QVariant(nAnonymizeMedicCoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeMedicCoinAmount:
            nAnonymizeMedicCoinAmount = value.toInt();
            settings.setValue("nAnonymizeMedicCoinAmount", nAnonymizeMedicCoinAmount);
            emit AnonymizeMedicCoinAmountChanged(nAnonymizeMedicCoinAmount);
            break;
        default:
            break;
        }
